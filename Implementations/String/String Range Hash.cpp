...
void build(int _len) {
    ...
    for(int it=0;it<2;it++) {
        for(int i=1;i<=len;i++) {
            h[it][i] = (h[it][i - 1] + str[i - 1] * pwr[it][i]) % mod[it];
        }
    }
}
...
ll get(int a, int b) {
    assert(a <= b);
    ll one = (h[0][b + 1] - h[0][a]) * inv[0][a + 1] % mod[0];
    ll two = (h[1][b + 1] - h[1][a]) * inv[1][a + 1] % mod[1];
...
int main() {
    ...
    for(int t=1;t<=T;t++) {
        int n; cin >> n >> str;
        build(n);
        //call get() or rv_hash() to get range hash
    }
    return 0;
}
